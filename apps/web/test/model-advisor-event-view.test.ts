  ...
  it("projects stale plan and artifact evidence without prose", () => {
    const event = advisorEvent("model.advisor.notice", {
      ...
      evidence: {
        ...
        latestGoalInvalidatedSeq: 14,
        latestRecoveryCompletedSeq: 15,
        latestRunInterruptedSeq: 16,
        latestRecoveryInvalidatedSeq: 18,
        latestEvaluationCompletedSeq: 19,
        latestEvaluationPassedSeq: 20,
        latestEvaluationPassInvalidatedSeq: 21,
      },
      contentSha256: "c".repeat(64),
    });
    ...
    expect(modelAdvisorEventTraceSummary(event)).toBe(
      `advisor / notice / status notice / source deterministic_stream_lint / turn user / diagnostics 1 / workspace-write / workspace-write-seq 17 / plan-completed / plan-completed-seq 12 / plan-invalidated-seq 13 / plan-completion-stale / artifact-verified / artifact-verified-seq 11 / artifact-invalidated-seq 13 / artifact-verification-stale / goal-satisfied / goal-satisfied-seq 10 / goal-invalidated-seq 14 / goal-satisfaction-stale / recovery-completed / recovery-completed-seq 15 / run-interrupted-seq 16 / recovery-invalidated-seq 18 / recovery-completion-stale / evaluation-completed / evaluation-completed-seq 19 / evaluation-completion-stale / evaluation-passed / evaluation-passed-seq 20 / evaluation-pass-invalidated-seq 21 / evaluation-pass-stale / text ${"a".repeat(12)} / diagnostics ${"b".repeat(12)} / receipt ${"c".repeat(12)}`,
    );
    expect(modelAdvisorEventTraceSummary(event)).not.toContain("TOP_SECRET");
  });
  ...
  it("projects independent reviews without issue guidance text", () => {
    const event = advisorEvent("model.advisor.independent.reviewed", {
      ...
      evidenceSummary: {
        ...
        evaluationCompleted: true,
        evaluationPassed: true,
        planCompletedAfterWorkspaceWrite: true,
        planArtifactVerifiedAfterWorkspaceWrite: true,
        goalSatisfiedAfterWorkspaceWrite: true,
        recoveryCompletedAfterInterruption: true,
        evaluationCompletedAfterWorkspaceWrite: true,
        evaluationPassedAfterWorkspaceWrite: true,
        latestPassedVerificationSeq: 22,
        latestPlanCompletedSeq: 21,
        latestPlanArtifactVerifiedSeq: 20,
        latestGoalSatisfiedSeq: 19,
        latestRecoveryCompletedSeq: 18,
        latestEvaluationCompletedSeq: 17,
        latestEvaluationPassedSeq: 16,
      },
