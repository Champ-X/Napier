...
const TURN_HEADER_HEIGHT_PX = 32;
export const TRACE_EVENT_ROW_HEIGHT_PX = 64;
export const TRACE_COMPACT_EVENT_ROW_HEIGHT_PX = 72;
...
export function createTraceVirtualLayout(
  ...
  for (const turn of collection.turns) {
    ...
    for (const row of turn.rows) {
      const height =
        row.kind === "event"
          ? (options.eventRowHeightPx ?? TRACE_EVENT_ROW_HEIGHT_PX)
          : FOLD_ROW_HEIGHT_PX;
      items.push({
        kind: "row",
        key: row.key,
