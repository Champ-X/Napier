...
export type ModelInvocationExperimentWebRequest = Omit<
  CreateModelInvocationExperimentRequest,
  "expectedPreviewSha256"
>;

export type ModelInvocationExperimentWebFrame =
  | StreamFrame
  | ModelInvocationExperimentResultFrame;
...
export async function previewModelInvocationExperiment(
  threadId: string,
  body: ModelInvocationExperimentWebRequest,
  signal?: AbortSignal,
  ...
  if (!response.ok) {
    await throwNapierApiError(
      response,
      "Model invocation experiment preview failed",
      path,
    );
  }
  ...
  try {
    ...
    throw new Error("Model invocation experiment preview response is invalid");
  }
  const preview = await validateModelInvocationExperimentPreview(input);
  if (
    preview.sourceThreadId !== threadId ||
    preview.sourceRunId !== body.sourceRunId ||
    preview.sourceTurnIndex !== body.sourceTurnIndex ||
    canonicalJson(preview.targetModel) !==
      canonicalJson(body.model ?? preview.sourceModel)
  ) {
    throw new Error("Model invocation experiment preview binding is invalid");
  }
  expectPreviewHeaders(response.headers, preview.previewSha256);
  return preview;
}

export async function executeModelInvocationExperiment(
  threadId: string,
  body: ModelInvocationExperimentWebRequest & {
    expectedPreviewSha256: string;
  },
  expectedPreview: ModelInvocationExperimentPreview,
  onFrame?: (frame: ModelInvocationExperimentWebFrame) => void,
  signal?: AbortSignal,
  ...
  if (
    ...
    body.sourceTurnIndex !== expectedPreview.sourceTurnIndex ||
    canonicalJson(body.model ?? expectedPreview.sourceModel) !==
      canonicalJson(expectedPreview.targetModel)
  ) {
    throw new Error("Model invocation experiment execution preview is stale");
  }
  const response = await fetch(path, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    await throwNapierApiError(
      response,
      "Model invocation experiment failed",
      path,
    );
  }
  ...
  if (!response.body) {
    throw new Error("Model invocation experiment stream is unavailable");
  }
  ...
  for await (const record of readSseJsonRecords(path, response.body, {
    ...
    if (terminal) {
      throw new Error(
        "Model invocation experiment stream emitted after terminal frame",
      );
    }
    const type = recordType(record.value);
    if (type === "model_invocation_experiment_result") {
      if (record.id !== undefined) {
        throw new Error(
          "Model invocation experiment result frame ID is invalid",
        );
      }
      if (record.eventType && record.eventType !== type) {
        throw new Error(
          "Model invocation experiment SSE event type is invalid",
        );
      }
      if (!snapshot) {
        throw new Error(
          "Model invocation experiment result is missing its snapshot",
        );
      }
      const result = await validateModelInvocationExperimentResultFrame(
        record.value,
      );
    ...
    if (frame.type === "done") {
      throw new Error(
        "Model invocation experiment stream used a Run done frame",
      );
    }
    if (frame.type === "event") {
      if (snapshot) {
        throw new Error(
          "Model invocation experiment emitted an event after snapshot",
        );
      }
      if (lastEventSeq !== undefined && frame.event.seq <= lastEventSeq) {
        throw new Error(
          "Model invocation experiment event sequence is not increasing",
        );
      }
      ...
      if (frame.event.threadId !== targetThreadId) {
        throw new Error(
          "Model invocation experiment event Thread binding is invalid",
        );
      }
      ...
      if (snapshot) {
        throw new Error(
          "Model invocation experiment emitted duplicate snapshots",
        );
      }
      targetThreadId ??= frame.detail.thread.id;
      if (frame.detail.thread.id !== targetThreadId) {
        throw new Error(
          "Model invocation experiment snapshot Thread binding is invalid",
        );
      }
      snapshot = frame;
    } else {
      ...
      if (frame.threadId !== expectedThreadId) {
        throw new Error(
          "Model invocation experiment error Thread binding is invalid",
        );
      }
      terminal = frame;
    }
  ...
  if (!terminal) {
    throw new Error(
      `Model invocation experiment stream ended without terminal frame (${String(frameCount)} frames)`,
    );
  }
  if (terminal.type === "error") {
    throw new Error(
      `Model invocation experiment failed (${terminal.diagnosticSha256.slice(0, 12)})`,
    );
  }
  return terminal;
}
...
async function assertResultBinding(input: {
  ...
  if (
    ...
    result.sourceTurnIndex !== input.preview.sourceTurnIndex ||
    ...
    throw new Error("Model invocation experiment terminal binding is invalid");
  }
  ...
  for (const [seq, expectedHash] of input.streamedEventHashes) {
    ...
    if (!event) {
      throw new Error(
        "Model invocation experiment snapshot omitted a streamed event",
      );
    }
    if ((await sha256Text(JSON.stringify(event))) !== expectedHash) {
      throw new Error(
        "Model invocation experiment snapshot changed a streamed event",
      );
    }
  }
  ...
  if (result.eventStreamSha256 !== eventStreamSha256) {
    throw new Error("Model invocation experiment event stream hash is invalid");
  }
}
...
async function readBoundedResponseText(
  ...
  for (;;) {
    ...
    if (totalBytes > maximumBytes) {
      throw new Error(
        "Model invocation experiment response exceeds its byte limit",
      );
    }
...
function expectHeader(headers: Headers, name: string, expected: string): void {
  if (headers.get(name) !== expected) {
    throw new Error(
      `Model invocation experiment response header is invalid: ${name}`,
    );
  }
}
