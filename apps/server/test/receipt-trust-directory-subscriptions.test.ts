...
const COSIGNING_ENV = "NAPIER_TEST_QUORUM_METADATA_COSIGNING_KEY";
const FOREIGN_SIGNING_ENV = "NAPIER_TEST_QUORUM_METADATA_FOREIGN_SIGNING_KEY";

afterEach(async () => {
  delete process.env[SIGNING_ENV];
  delete process.env[COSIGNING_ENV];
...
describe("receipt trust anchor directory subscription HTTP surface", () => {
  it("promotes only valid CAS-bound refreshes and retains last-good trust", async () => {
    ...
    const freshRotationProposalSourceUrl =
      "https://mirror.example.test/napier/activation-selection-fresh-rotation-proposal.json";
    ...
    const services = await createNapierServices({
      ...
      receiptTrustDirectoryDiscovery: {
        ...
        fetcher: async (input) => {
          ...
          if (
            input === rotationProposalSourceUrl ||
            input === freshRotationProposalSourceUrl
          ) {
    ...
    const { privateKey: coSigningPrivateKey } = generateKeyPairSync("ed25519");
    process.env[COSIGNING_ENV] = coSigningPrivateKey
      .export({ format: "pem", type: "pkcs8" })
      .toString();
    const coSigningAnchorResponse = await app.request(
      "/api/receipt-trust/anchors",
      {
        ...
        body: JSON.stringify({
          threadId: thread.id,
          label: "Hosted verifier A co-signer",
          source: { type: "environment", variable: COSIGNING_ENV },
        }),
      },
    );
    expect(coSigningAnchorResponse.status).toBe(201);
    const coSigningAnchor =
      (await coSigningAnchorResponse.json()) as ReceiptTrustAnchor;
    ...
    expect(
      validCheckpointDiscoveryResponse.headers.get("x-napier-discovery-status"),
    ).toBe("valid");
    expect(
      validCheckpointDiscoveryResponse.headers.get("x-napier-content-sha256"),
    ...
    const emptyCheckpointRegistryQuorumBaselineListResponse = await app.request(
    ...
    const rotationProposalDiscoveryPolicy = {
    ...
    const signedRotationProposalDiscoveryResponse = await app.request(
      `${rotationProposalPath}/discover`,
      {
        ...
        body: JSON.stringify({
          ...
          policy: rotationProposalDiscoveryPolicy,
        }),
      },
    );
    ...
    const rotationProposalSubscriptionPath = `${rotationProposalPath}/subscriptions`;
    const createRotationProposalSubscriptionResponse = await app.request(
      rotationProposalSubscriptionPath,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          threadId: thread.id,
          label: "Hosted rotation proposal feed",
          sourceUrl: rotationProposalSourceUrl,
          refreshIntervalMs: 5 * 60 * 1_000,
          policy: rotationProposalDiscoveryPolicy,
        }),
    ...
    expect(createRotationProposalSubscriptionResponse.status).toBe(201);
    const rotationProposalSubscription =
      (await createRotationProposalSubscriptionResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscription;
    expect(rotationProposalSubscription).toEqual(
      expect.objectContaining({
        ...
        sourceUrlSha256: sha256Text(rotationProposalSourceUrl),
        sourceOriginSha256: sha256Text("https://trust.example.test"),
        policySha256: signedRotationProposalDiscovery.policySha256,
        lastRefreshStatus: "accepted",
        ...
        transparencyHistory: [
          expect.objectContaining({
            sequence: 1,
            status: "accepted",
            discoverySha256: expect.stringMatching(/^[a-f0-9]{64}$/),
            envelopeSha256: signedRotationProposal.contentSha256,
            proposalSha256: signedRotationProposal.receipt.contentSha256,
          }),
        ],
    ...
    expect(rotationProposalSubscription.lastDiscoverySha256).toBe(
      rotationProposalSubscription.lastGoodDiscovery?.contentSha256,
    );
    expect(
      rotationProposalSubscription.transparencyHistory[0]?.discoverySha256,
    ).toBe(rotationProposalSubscription.lastGoodDiscovery?.contentSha256);
    expect(JSON.stringify(rotationProposalSubscription)).not.toContain(
      rotationProposalSourceUrl,
    );
    expect(
      createRotationProposalSubscriptionResponse.headers.get(
        "x-napier-receipt-trust-directory-quorum-activation-selection-rotation-proposal-subscription-sha256",
      ),
    ).toBe(rotationProposalSubscription.contentSha256);
    expect(
      createRotationProposalSubscriptionResponse.headers.get(
        "x-napier-receipt-trust-directory-quorum-activation-selection-rotation-proposal-subscription-last-refresh-status",
      ),
    ).toBe("accepted");
    const listRotationProposalSubscriptionsResponse = await app.request(
      rotationProposalSubscriptionPath,
    );
    expect(listRotationProposalSubscriptionsResponse.status).toBe(200);
    expect(
      (await listRotationProposalSubscriptionsResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscription[],
    ).toEqual([rotationProposalSubscription]);
    expect(
      listRotationProposalSubscriptionsResponse.headers.get(
        "x-napier-receipt-trust-directory-quorum-activation-selection-rotation-proposal-subscription-count",
      ),
    ).toBe("1");
    const unchangedRotationProposalSubscriptionRefreshResponse =
      await app.request(
        `${rotationProposalSubscriptionPath}/${rotationProposalSubscription.id}/refresh`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            threadId: thread.id,
            expectedRevision: rotationProposalSubscription.revision,
          }),
        },
      );
    expect(unchangedRotationProposalSubscriptionRefreshResponse.status).toBe(
      200,
    );
    const unchangedRotationProposalSubscriptionRefresh =
      (await unchangedRotationProposalSubscriptionRefreshResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionRefreshResult;
    expect(unchangedRotationProposalSubscriptionRefresh).toEqual(
      expect.objectContaining({
        status: "unchanged",
    ...
    expect(
      unchangedRotationProposalSubscriptionRefreshResponse.headers.get(
    ...
    const rejectedRotationProposalSubscriptionRefreshResponse =
      await app.request(
        `${rotationProposalSubscriptionPath}/${rotationProposalSubscription.id}/refresh`,
        {
          method: "POST",
    ...
    expect(rejectedRotationProposalSubscriptionRefreshResponse.status).toBe(
      200,
    );
    const rejectedRotationProposalSubscriptionRefresh =
      (await rejectedRotationProposalSubscriptionRefreshResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionRefreshResult;
    expect(rejectedRotationProposalSubscriptionRefresh).toEqual(
      expect.objectContaining({
        status: "rejected",
        subscription: expect.objectContaining({
          revision: 3,
    ...
    hostedRotationProposalEnvelope = undefined;
    const failedRotationProposalSubscriptionRefreshResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${rotationProposalSubscription.id}/refresh`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          threadId: thread.id,
          expectedRevision:
            rejectedRotationProposalSubscriptionRefresh.subscription.revision,
        }),
      },
    );
    expect(failedRotationProposalSubscriptionRefreshResponse.status).toBe(200);
    const failedRotationProposalSubscriptionRefresh =
      (await failedRotationProposalSubscriptionRefreshResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionRefreshResult;
    expect(failedRotationProposalSubscriptionRefresh).toEqual(
      expect.objectContaining({
        status: "failed",
        ...
        subscription: expect.objectContaining({
          ...
          lastGoodDiscovery:
            rejectedRotationProposalSubscriptionRefresh.subscription
              .lastGoodDiscovery,
        }),
      }),
    );
    expect(
      JSON.stringify(failedRotationProposalSubscriptionRefresh),
    ).not.toContain("Rotation proposal envelope is unavailable");
    const pauseRotationProposalSubscriptionResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${rotationProposalSubscription.id}`,
      {
        ...
        body: JSON.stringify({
          threadId: thread.id,
          expectedRevision:
            failedRotationProposalSubscriptionRefresh.subscription.revision,
          status: "paused",
        }),
      },
    );
    ...
    const checkpointRegistryQuorumImportServices = await createNapierServices({
      dataRoot: path.join(checkpointRegistryQuorumImportRoot, "data"),
      workspaceRoot: path.join(checkpointRegistryQuorumImportRoot, "workspace"),
    });
    ...
    expect(failedCheckpointSubscriptionRefresh).toEqual(
      expect.objectContaining({
        ...
        subscription: expect.objectContaining({
          ...
          lastRefreshStatus: "failed",
          lastGoodDiscovery:
            rejectedCheckpointSubscriptionRefresh.subscription
              .lastGoodDiscovery,
        }),
      }),
    );
    ...
    const staleApprovalApplyResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${rotationProposalSubscription.id}/approval/apply`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          expectedSubscriptionSha256:
            unchangedRotationProposalSubscriptionRefresh.subscription
              .contentSha256,
          approvalEnvelope: rotationProposalApprovalEnvelope,
        }),
      },
    );
    expect(staleApprovalApplyResponse.status).toBe(409);
    expect(await staleApprovalApplyResponse.json()).toEqual(
      expect.objectContaining({
        error: expect.stringContaining("approval revision changed"),
      }),
    );
    ...
    hostedRotationProposalEnvelope = freshSignedRotationProposal;
    const freshRotationProposalSubscriptionResponse = await app.request(
      rotationProposalSubscriptionPath,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          threadId: thread.id,
          label: "Fresh hosted rotation proposal feed",
          sourceUrl: freshRotationProposalSourceUrl,
          refreshIntervalMs: 5 * 60 * 1_000,
          policy: {
            maxEnvelopeAgeMs: 24 * 60 * 60 * 1_000,
    ...
    expect(freshRotationProposalSubscriptionResponse.status).toBe(201);
    const freshRotationProposalSubscription =
      (await freshRotationProposalSubscriptionResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscription;
    expect(freshRotationProposalSubscription).toEqual(
      expect.objectContaining({
        label: "Fresh hosted rotation proposal feed",
        sourceUrlSha256: sha256Text(freshRotationProposalSourceUrl),
        sourceOriginSha256: sha256Text("https://mirror.example.test"),
        lastRefreshStatus: "accepted",
        lastGoodDiscovery: expect.objectContaining({
          envelopeSha256: freshSignedRotationProposal.contentSha256,
    ...
    expect(freshRotationApprovalResponse.status).toBe(201);
    const freshRotationApprovalEnvelope =
      (await freshRotationApprovalResponse.json()) as TrustedReceiptEnvelope<ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionApproval>;
    expect(freshRotationApprovalEnvelope).toEqual(
      expect.objectContaining({
        receiptKind:
          "receipt_trust_anchor_directory_quorum_activation_selection_rotation_proposal_subscription_approval",
        receipt: expect.objectContaining({
    ...
    const freshRotationCoApprovalResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}/approval/sign`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          threadId: thread.id,
          trustAnchorId: coSigningAnchor.id,
          expectedSubscriptionRevision:
            freshRotationProposalSubscription.revision,
          expectedSubscriptionSha256:
            freshRotationProposalSubscription.contentSha256,
    ...
    expect(freshRotationCoApprovalResponse.status).toBe(201);
    const freshRotationCoApprovalEnvelope =
      (await freshRotationCoApprovalResponse.json()) as TrustedReceiptEnvelope<ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionApproval>;
    expect(freshRotationCoApprovalEnvelope.signature.keyId).toBe(
      coSigningAnchor.keyId,
    );
    const approvalPolicy = {
      minimumDistinctSignerCount: 2,
      requiredSignerKeyIds: [coSigningAnchor.keyId, signingAnchor.keyId].sort(),
    };
    const rejectedPolicyReviewResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}/approval/policy-review`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          expectedSubscriptionSha256:
            freshRotationProposalSubscription.contentSha256,
          approvalEnvelopes: [freshRotationApprovalEnvelope],
          approvalPolicy,
        }),
      },
    );
    expect(rejectedPolicyReviewResponse.status).toBe(200);
    expect(
      (await rejectedPolicyReviewResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionApprovalPolicyReview,
    ).toEqual(
      expect.objectContaining({
        status: "rejected",
        diagnostics: expect.arrayContaining([
          "approval_distinct_signer_count_below_policy",
          "required_signer_missing",
        ]),
    ...
    const acceptedPolicyReviewResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}/approval/policy-review`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    expect(acceptedPolicyReviewResponse.status).toBe(200);
    const acceptedPolicyReview =
      (await acceptedPolicyReviewResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionApprovalPolicyReview;
    expect(acceptedPolicyReview).toEqual(
      expect.objectContaining({
        status: "accepted",
        diagnostics: [],
        acceptedApprovalCount: 2,
        distinctSignerCount: 2,
        requiredSignerCount: 2,
        acceptedApprovalSignerKeyIds: approvalPolicy.requiredSignerKeyIds,
        proposalSha256: freshSignedRotationProposal.receipt.contentSha256,
      }),
    );
    expect(
      acceptedPolicyReviewResponse.headers.get(
    ...
    expect(prePolicyApplySelectionResponse.status).toBe(200);
    expect(
      (await prePolicyApplySelectionResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionState,
    ).toEqual(
      expect.objectContaining({
        hasSelection: true,
        selection: expect.objectContaining({
    ...
    const driftApprovalPolicy = {
      minimumDistinctSignerCount: 1,
    };
    const driftApprovalPolicyBaselineResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}/approval/policy-baselines`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          approvalPolicy: driftApprovalPolicy,
        }),
      },
    );
    expect(driftApprovalPolicyBaselineResponse.status).toBe(201);
    const driftApprovalPolicyBaselineResult =
      (await driftApprovalPolicyBaselineResponse.json()) as PromoteReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionApprovalPolicyBaselineResult;
    expect(
      driftApprovalPolicyBaselineResult.baseline.approvalPolicySha256,
    ).not.toBe(approvalPolicyBaselineResult.baseline.approvalPolicySha256);
    const driftPolicyApplyQueueResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}/approval/policy-apply/queue`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          approvalPolicyBaselineSha256:
            driftApprovalPolicyBaselineResult.baseline.contentSha256,
        }),
      },
    );
    expect(driftPolicyApplyQueueResponse.status).toBe(409);
    expect(await driftPolicyApplyQueueResponse.text()).toContain(
      "approval_policy_baseline_policy_mismatch",
    );
    const policyApplyAfter = new Date(Date.now() + 1_000).toISOString();
    const policyApplyQueueResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}/approval/policy-apply/queue`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          approvalPolicy,
          approvalPolicyBaselineSha256:
            approvalPolicyBaselineResult.baseline.contentSha256,
          applyAfter: policyApplyAfter,
        }),
      },
    );
    expect(policyApplyQueueResponse.status).toBe(202);
    const policyApplyQueue =
      (await policyApplyQueueResponse.json()) as QueueReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptionApprovalPolicyApplyResult;
    expect(policyApplyQueue).toEqual(
      expect.objectContaining({
        ...
        subscription: expect.objectContaining({
          id: freshRotationProposalSubscription.id,
          contentSha256: freshRotationProposalSubscription.contentSha256,
        }),
      }),
    );
    expect(
      policyApplyQueueResponse.headers.get("x-napier-verification-status"),
    ).toBe("accepted");
    expect(JSON.stringify(policyApplyQueue)).not.toContain("pendingApproval");
    expect(
      await services.receiptTrustDirectorySubscriptions.refreshDue(
        new Date(Date.parse(policyApplyAfter) - 1),
      ),
    ).toBe(0);
    expect(
      await services.receiptTrustDirectorySubscriptions.refreshDue(
    ...
    const pauseFreshRotationProposalSubscriptionResponse = await app.request(
      `${rotationProposalSubscriptionPath}/${freshRotationProposalSubscription.id}`,
      {
        ...
        body: JSON.stringify({
          threadId: thread.id,
          expectedRevision: currentFreshRotationProposalSubscription.revision,
          status: "paused",
        }),
      },
    );
    if (pauseFreshRotationProposalSubscriptionResponse.status !== 200) {
      throw new Error(
        await pauseFreshRotationProposalSubscriptionResponse.text(),
      );
    }
    expect(pauseFreshRotationProposalSubscriptionResponse.status).toBe(200);
    expect(
      (await pauseFreshRotationProposalSubscriptionResponse.json()) as ReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscription,
    ).toEqual(
      expect.objectContaining({
        status: "paused",
        revision: currentFreshRotationProposalSubscription.revision + 1,
      }),
    );
    ...
    const dueRefreshedRotationProposalSubscription = services.store
      .listReceiptTrustAnchorDirectoryQuorumActivationSelectionRotationProposalSubscriptions()
      .find(
        (subscription) => subscription.id === rotationProposalSubscription.id,
      );
    ...
    expect(
      events.filter((event) =>
        event.type.startsWith("receipt.trust_rotation_proposal_subscription."),
      ),
    ).toHaveLength(9);
    expect(
      events.filter((event) =>
        event.type.startsWith(
          "receipt.trust_rotation_proposal_approval_apply.",
        ),
      ),
    ).toHaveLength(0);
    expect(
      events.filter((event) =>
        event.type.startsWith(
          "receipt.trust_rotation_proposal_approval_policy_apply.",
        ),
      ),
    ).toHaveLength(2);
    expect(
    ...
    expect(JSON.stringify(events)).not.toContain(
      freshRotationProposalSourceUrl,
    );
    expect(JSON.stringify(events)).not.toContain("private upstream detail");
    expect(JSON.stringify(events)).not.toContain(
      "Rotation proposal envelope is unavailable",
    );
  });
});
