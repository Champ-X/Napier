...
describe("SDK capability parity evidence verifier", () => {
  ...
  test("excludes protected user files from the repair snapshot", async () => {
    ...
    expect(isProtectedExcludedPath("pre.md")).toBe(true);
    expect(isProtectedExcludedPath("next.md")).toBe(true);
  ...
  test("locks the deterministic execution closures and exclusions", async () => {
    ...
    expect(fourStateParity.counts).toEqual({
      executionFiles: 1186,
      sourceCounterparts: 1181,
      packageManifests: 2,
      allFiles: 2369,
    });
    expect(fourStateParity.executionAreaCounts).toEqual({
      "apps/cli/dist": 67,
      "apps/server/dist": 137,
      "packages/contracts/dist": 57,
      "packages/runtime/dist": 917,
    ...
    expect(productionServerTrace.counts).toEqual({
      executionFiles: 1119,
      sourceCounterparts: 1113,
      packageManifests: 3,
      allFiles: 2235,
    });
    expect(productionServerTrace.executionAreaCounts).toEqual({
      "apps/server/dist": 139,
      other: 1,
      "packages/contracts/dist": 57,
      "packages/runtime/dist": 914,
      "packages/sdk/dist": 3,
      scripts: 5,
    });
    expect(
