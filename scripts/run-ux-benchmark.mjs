...
if (args.verifySeries) {
  ...
  const artifacts = [];
  for (const reference of uxBenchmarkSeriesArtifactReferences(series)) {
    artifacts.push({
  ...
  const verification = verifyUxBenchmarkSeries(series, artifacts);
  print(verification);
  ...
  try {
    const options = {
      caseRoot:
        args.caseRoot ?? path.join(repoRoot, "benchmarks/ux/first-task-cli-v1"),
    ...
    if ((args.trialCount ?? 1) > 1) {
      ...
      const artifacts = await runUxBenchmark(options);
      print({
        status: artifacts.result.status,
        caseId: artifacts.result.caseId,
        model: artifacts.result.model,
        runDurationMs: artifacts.result.run.durationMs,
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!allowed.has(flag)) throw new Error("Unknown UX benchmark option");
    if (values.has(flag)) {
      throw new Error(`Duplicate UX benchmark option: ${flag}`);
    }
...
async function readJson(filePath, maximumBytes) {
  ...
  if (!info.isFile() || info.isSymbolicLink() || info.size > maximumBytes) {
    throw new Error("UX benchmark artifact is unsafe");
  }
