...
describe("Workflow preview-bound Git branch Tool nodes", () => {
  it("passes one Plan-scoped branch preview without durable names", async () => {
    ...
    const manifest = defineExecutionPlanWorkflow({
      name: "Create reviewed local branch",
      version: 1,
      description: "Preview and create one branch at current HEAD.",
      ...
      nodes: [
        {
          id: "preview",
          type: "tool",
          tool: "git_branch_create_preview",
          effect: "read",
          inputBindings: {
            branchName: {
              source: "literal",
              value: "feature/PRIVATE_WORKFLOW_BRANCH",
            },
          },
    ...
    const result = await workflows.run({
      ...
      request: {
        manifest,
        input: { request: "Create the reviewed local branch." },
      },
    });
    ...
    expect(
      (
        await gitOutput(fixture.workspaceRoot, [
          "rev-parse",
          "refs/heads/feature/PRIVATE_WORKFLOW_BRANCH",
        ])
      ).trim(),
    ).toBe(parent);
    ...
    expect(durable).not.toContain("PRIVATE_WORKFLOW_BRANCH");
...
async function createFixture() {
  const root = await mkdtemp(
    path.join(tmpdir(), "napier-workflow-git-branch-"),
  );
  roots.push(root);
  ...
  const plan = await store.createPlan(sourceThread.id, {
    objective: "Create one reviewed local branch.",
    steps: [
      {
        id: "preview",
        title: "Preview branch",
        description: "Bind a new branch to current HEAD.",
        verification: "Return one Plan-scoped capability.",
      },
      {
        id: "apply",
        title: "Create branch",
        description: "CAS-create the exact local ref.",
        verification: "Return a verified durable ref receipt.",
        dependsOn: ["preview"],
      },
    ],
  });
  ...
  const thread = await store.createThread({
    title: "Git branch Workflow target",
...
function receiptSchema(action: "preview" | "apply"): WorkflowObjectSchema {
  ...
  const properties = {
    kind: { type: "string", enum: ["napier.git-branch"] } as const,
    schemaVersion: { type: "integer", minimum: 1, maximum: 1 } as const,
    operation: { type: "string", enum: ["create"] } as const,
...
function directSandbox(): OsSandboxAdapter {
  return {
    id: "direct-workflow-git-branch-test",
