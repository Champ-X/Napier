...
import {
  ExecutionPlanWorkflowJavascriptError,
  ExecutionPlanWorkflowJavascriptRuntime,
} from "./workflow-javascript-runtime.js";
import {
    ...
    try {
      ...
      return {
        result: await this.operations.blockNode(context, node, {
          ...(error instanceof ExecutionPlanWorkflowJavascriptError && error.run
            ? { runId: error.run.id }
            : runId
              ? { runId }
              : {}),
