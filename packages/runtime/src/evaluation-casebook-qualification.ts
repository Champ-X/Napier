...
import { RunEvaluationService, type RunEvaluationJudgment, type RunEvaluationTraceOptions } from "./evaluation.js";
import { MAX_EVALUATION_CASEBOOK_ARTIFACT_BYTES, currentCasebookRevision, validateEvaluationCasebook } from "./evaluation-casebooks.js";
import { assertEvaluationCasebookTemplateCoverage } from "./evaluation-casebook-templates.js";
...
const VERDICTS = new Set<RunEvaluationVerdict>(["left_better", "right_better", "tie", "inconclusive"]);
...
export class EvaluationCasebookQualificationService {
  ...
  async execute(casebookId: string, request: ExecuteEvaluationCasebookRequest): Promise<EvaluationCasebookQualificationExecution> {
    const auditThread = this.store.getThread(request.threadId);
    const casebook = this.store.getEvaluationCasebook(casebookId);
    assertEvaluationCasebookTemplateCoverage(casebook);
    ...
    try {
      ...
      const inconclusiveCount = results.filter((item) => item.actualVerdict === "inconclusive").length;
      const unverifiedCount = results.filter((item) => item.evidenceState !== "verified").length;
      const agreementRate = sampleCount > 0 ? Number((agreementCount / sampleCount).toFixed(4)) : 0;
      const status = qualificationStatus(gate, sampleCount, agreementRate, inconclusiveCount, unverifiedCount);
      const evidence = {
      ...
      const saved = await this.store.saveEvaluationCasebookQualificationExecution(execution);
      ...
      await this.store.finishRun(qualificationRun.id, "completed", {
        usage: await this.collectRunUsage(request.threadId, qualificationRun.id),
      });
      return saved;
    } catch (error) {
  ...
  private async evaluateCase(
    ...
    try {
      [left, right] = await Promise.all([
        createRunReplaySnapshot(this.store, item.sourceThreadId, item.evaluation.leftRunId),
        createRunReplaySnapshot(this.store, item.sourceThreadId, item.evaluation.rightRunId),
      ]);
    } catch (error) {
      return failedCaseResult(item, "missing", `Casebook source evidence is unavailable: ${safeErrorMessage(error)}`);
    }
    const observed = {
      observedLeftSnapshotSha256: left.eventStreamSha256,
      observedRightSnapshotSha256: right.eventStreamSha256,
    };
    if (left.eventStreamSha256 !== item.evaluation.leftSnapshotSha256 || right.eventStreamSha256 !== item.evaluation.rightSnapshotSha256) {
      return failedCaseResult(item, "drifted", "Casebook source evidence no longer matches its curated snapshot hashes.", observed);
    }
    const judgment = await this.evaluator.judgeSnapshots(
      ...
      evaluatorModel,
      item.evaluation.comparisonGovernance ? { comparisonGovernance: item.evaluation.comparisonGovernance } : undefined,
      nextTrace(),
    );
  ...
  private async collectRunUsage(threadId: string, runId: string) {
    return aggregateRunUsage(
      (await this.store.listEvents(threadId)).filter((event) => event.runId === runId),
      [],
    );
  }
}
...
export function normalizeEvaluationCasebookQualificationGate(
  ...
  if (!Number.isFinite(gate.minimumAgreementRate) || gate.minimumAgreementRate < 0 || gate.minimumAgreementRate > 1) {
    throw new Error("Evaluation Casebook qualification agreement rate must be 0-1");
  }
  if (typeof gate.allowInconclusive !== "boolean") {
    throw new Error("Evaluation Casebook qualification inconclusive policy is invalid");
  }
  return gate;
}

export function hashEvaluationCasebookQualificationExecution(
  input: Omit<EvaluationCasebookQualificationExecution, "id" | "contentSha256" | "startedAt" | "finishedAt">,
): string {
...
export function validateEvaluationCasebookQualificationExecution(
  ...
  const revision = casebook.revisions.find((candidate) => candidate.revision === input.casebookRevision);
  ...
  if (
    ...
    JSON.stringify(input.gate) !== JSON.stringify(normalizeEvaluationCasebookQualificationGate(input.gate)) ||
  ...
  const inconclusiveCount = input.results.filter((item) => item.actualVerdict === "inconclusive").length;
  const unverifiedCount = input.results.filter((item) => item.evidenceState !== "verified").length;
  const agreementRate = sampleCount > 0 ? Number((agreementCount / sampleCount).toFixed(4)) : 0;
  const status = qualificationStatus(input.gate, sampleCount, agreementRate, inconclusiveCount, unverifiedCount);
  if (
  ...
  const { id: _id, contentSha256: _contentSha256, startedAt: _startedAt, finishedAt: _finishedAt, ...content } = input;
  if (hashEvaluationCasebookQualificationExecution(content) !== input.contentSha256) {
    throw new Error("Evaluation Casebook qualification execution hash mismatch");
  }
...
export function createEvaluationCasebookQualificationReceipt(
  ...
  const execution = store
    ...
    .filter((candidate) => candidate.casebookRevision === casebook.currentRevision)
    .at(-1);
  const content: Omit<EvaluationCasebookQualificationReceipt, "generatedAt" | "contentSha256"> = {
  ...
  if (Buffer.byteLength(JSON.stringify(receipt)) > MAX_EVALUATION_CASEBOOK_ARTIFACT_BYTES) {
    throw new Error(`Evaluation Casebook qualification receipt exceeds ${MAX_EVALUATION_CASEBOOK_ARTIFACT_BYTES} bytes`);
  }
  return receipt;
}

export function hashEvaluationCasebookQualificationReceipt(
  input: Omit<EvaluationCasebookQualificationReceipt, "generatedAt" | "contentSha256">,
): string {
  return sha256(canonicalJson(input));
}

export function validateEvaluationCasebookQualificationReceipt(value: unknown): EvaluationCasebookQualificationReceipt {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("Evaluation Casebook qualification receipt must be an object");
  }
  const receipt = value as EvaluationCasebookQualificationReceipt;
  if (
    Buffer.byteLength(JSON.stringify(value)) > MAX_EVALUATION_CASEBOOK_ARTIFACT_BYTES ||
  ...
  if (receipt.execution) {
    validateEvaluationCasebookQualificationExecution(receipt.execution, casebook);
    if (receipt.execution.casebookRevision !== casebook.currentRevision || receipt.state !== receipt.execution.status) {
      throw new Error("Evaluation Casebook qualification receipt execution is stale");
    }
  } else if (receipt.state !== "not_run") {
    throw new Error("Evaluation Casebook qualification receipt state is invalid");
  }
  const { generatedAt: _generatedAt, contentSha256: _contentSha256, ...content } = receipt;
  if (hashEvaluationCasebookQualificationReceipt(content) !== receipt.contentSha256) {
...
function qualificationCaseResult(
  ...
  const agreement = judgment.verdict === item.adjudicationRevision.expectedVerdict;
  return {
    ...
    ...observed,
    status: judgment.verdict === "inconclusive" ? "inconclusive" : agreement ? "agreed" : "disagreed",
  };
}
...
function validateQualificationCaseResult(result: EvaluationCasebookQualificationCaseResult, item: EvaluationCasebookCase): void {
  if (!result || typeof result !== "object" || Array.isArray(result)) {
    throw new Error(`Evaluation Casebook qualification judgment is invalid: ${item.id}`);
  }
  const evidence = caseResultEvidence(item);
  for (const [key, expected] of Object.entries(evidence)) {
    if (result[key as keyof EvaluationCasebookQualificationCaseResult] !== expected) {
      throw new Error(`Evaluation Casebook qualification case evidence is invalid: ${item.id}`);
    }
  }
  const expectedAgreement = result.evidenceState === "verified" && result.actualVerdict === result.expectedVerdict;
  if (
  ...
  if (
    ...
    throw new Error(`Evaluation Casebook qualification observed evidence is invalid: ${item.id}`);
  }
  if (
    result.evidenceState === "verified" &&
    (observedLeft !== result.expectedLeftSnapshotSha256 || observedRight !== result.expectedRightSnapshotSha256)
  ) {
    throw new Error(`Evaluation Casebook qualification verified evidence is invalid: ${item.id}`);
  }
  if (
    ...
    throw new Error(`Evaluation Casebook qualification drift evidence is invalid: ${item.id}`);
  }
  if (result.evidenceState === "missing" && (observedLeft !== undefined || observedRight !== undefined)) {
    throw new Error(`Evaluation Casebook qualification missing evidence is invalid: ${item.id}`);
  }
  if (
    result.evidenceState !== "verified" &&
    (result.actualVerdict !== "inconclusive" || result.agreement || result.scores.length > 0 || result.evidence.length > 0)
  ) {
    throw new Error(`Evaluation Casebook qualification unverified judgment is invalid: ${item.id}`);
  }
  validateQualificationScores(result, item);
  const expectedStatus =
    result.evidenceState !== "verified" || result.actualVerdict === "inconclusive"
      ? "inconclusive"
      : result.agreement
        ? "agreed"
        : "disagreed";
  if (result.status !== expectedStatus) {
    throw new Error(`Evaluation Casebook qualification case status is invalid: ${item.id}`);
  }
}

function validateQualificationScores(result: EvaluationCasebookQualificationCaseResult, item: EvaluationCasebookCase): void {
  if (result.evidenceState !== "verified" || result.actualVerdict === "inconclusive") {
    if (result.scores.length !== 0) {
      throw new Error(`Evaluation Casebook qualification scores are invalid: ${item.id}`);
    }
    return;
  }
...
function qualificationStatus(
  ...
  if (sampleCount === 0 || unverifiedCount > 0 || (!gate.allowInconclusive && inconclusiveCount > 0)) {
    return "inconclusive";
  }
...
function assertAvailableEvaluator(models: ModelRegistry, model: ModelRef): void {
  if (!(model.provider === "napier" && model.id === "demo") && !models.resolve(model)) {
