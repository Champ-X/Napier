...
const describeLive =
  process.env["NAPIER_LIVE_WORKFLOW_BENCHMARK"] === "1"
    ? describe
    : describe.skip;
const roots: string[] = [];
...
describeLive("live DeepSeek Workflow outcome benchmark", () => {
  it("passes typed Map Reduce with portable evidence", async () => {
    const apiKey = process.env["DEEPSEEK_API_KEY"]?.trim();
    if (!apiKey) {
      throw new Error(
        "Set DEEPSEEK_API_KEY before running the live Workflow benchmark",
      );
    }
    const outputDir = await mkdtemp(
      path.join(tmpdir(), "napier-live-workflow-benchmark-"),
    );
