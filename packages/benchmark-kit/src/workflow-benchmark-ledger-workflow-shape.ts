...
import { validWorkflowBenchmarkBudgetFields } from "./workflow-benchmark-budget-evidence.js";
...
export function workflowBenchmarkLedgerWorkflowDiagnostics(
  ...
  if (!validWorkflowBenchmarkBudgetFields(workflow)) {
    diagnostics.push("workflow_budget_fields_invalid");
  }
...
function workflowKeys(workflow: Record<string, unknown>): readonly string[] {
  return [
    ...
    ...(workflow["preRestartMapRunIds"] === undefined
      ? []
      : ["preRestartMapRunIds"]),
    ...(workflow["budgetExhaustionEvents"] === undefined
      ? []
