  ...
  return new BenchmarkCampaignRunner(options.outputDir).withWorkspace(
    ...
    async ({ workspaceRoot, dataRoot }) => {
      ...
      try {
        ...
        const eventsBeforeEvaluation = await runtime.store.listEvents(
          thread.id,
        );
        ...
        const dataFrameActionEvents =
          collectWorkflowBenchmarkDataFrameActionEvents(
            eventsBeforeEvaluation,
            new Set(mapRuns.map((run) => run.id)),
          );
        ...
        const evaluation = createWorkflowBenchmarkEvaluation({
          ...
          ...(workflowResult.outputSha256
            ...
            : {}),
          expectedMapOutputSha256: sha256(
            canonicalJson(loaded.expected.mapItems),
          ),
        ...
        return new BenchmarkCampaignRunner(options.outputDir).persistArtifacts<
          typeof bundle,
          ReturnType<typeof createWorkflowBenchmarkResult>
        >({
          bundle,
          ledgerFileName,
          ...
          verify: verifyWorkflowBenchmarkArtifacts,
          verificationError:
            "Workflow benchmark artifacts failed self-verification",
        });
      } finally {
