...
async function writeCodingBenchmarkArtifacts(input: {
  ...
  const artifacts = await new BenchmarkCampaignRunner(
    input.options.outputDir,
  ).persistArtifacts<
    typeof input.bundle,
    ReturnType<typeof createCodingBenchmarkResult>
  >({
    bundle: input.bundle,
    ledgerFileName,
    ...
    resultFileName: (result) =>
      `napier-benchmark-result-${input.benchmarkCaseId}-${result.contentSha256.slice(0, 16)}.json`,
    verify: verifyCodingBenchmarkArtifacts,
    verificationError: "Coding benchmark artifacts failed self-verification",
  });
  return {
