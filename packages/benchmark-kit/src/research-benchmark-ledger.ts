...
import {
  researchBenchmarkLedgerShapeDiagnostics,
  validResearchBenchmarkLedgerShape,
} from "./research-benchmark-ledger-shape.js";
import type {
  ResearchBenchmarkLedgerBundle,
  ResearchBenchmarkLedgerEventReceipt,
...
export function createResearchBenchmarkLedgerBundle(input: {
  generatedAt: string;
  caseId: string;
  caseSha256: string;
  threadId: string;
  ...
  sourceAuthorities: ResearchBenchmarkLedgerBundle["sourceAuthorities"];
  ...
  const content = {
    kind: "napier.research-benchmark-ledger" as const,
    ...
    ...(input.actualClaimsSha256
      ...
      : {}),
    contradictionClaimSha256: input.contradictionClaimSha256,
    ...
    sourceAuthorities: input.sourceAuthorities
      ...
      .sort((left, right) =>
        left.sourceContentSha256.localeCompare(right.sourceContentSha256),
      ),
    ...
    researchEvents: input.researchEvents
...
export function verifyResearchBenchmarkLedgerBundle(input: unknown): {
  valid: boolean;
  diagnostics: string[];
  bundleSha256: string;
} {
  const diagnostics: string[] = [];
  if (!validResearchBenchmarkLedgerShape(input)) {
    const shapeDiagnostics = researchBenchmarkLedgerShapeDiagnostics(input);
    return {
      valid: false,
      diagnostics: [
        "ledger_shape_invalid",
        ...shapeDiagnostics.map((diagnostic) => `ledger_shape:${diagnostic}`),
      ],
...
function validReceipts(bundle: ResearchBenchmarkLedgerBundle): boolean {
...
function validEventBindings(bundle: ResearchBenchmarkLedgerBundle): boolean {
  return [
    bundle.evaluationEvent,
    bundle.terminalEvent,
    ...bundle.researchEvents,
  ].every((event) => {
...
function validEventAggregates(bundle: ResearchBenchmarkLedgerBundle): boolean {
  return (
