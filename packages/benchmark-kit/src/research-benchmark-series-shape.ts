...
import type { ResearchBenchmarkSeries } from "./research-benchmark-types.js";

export function validResearchBenchmarkSeriesShape(
  value: unknown,
): value is ResearchBenchmarkSeries {
...
function validIdentity(value: Record<string, unknown>): boolean {
  return (
    value["kind"] === "napier.research-benchmark-series" &&
...
function safeArtifactName(value: unknown, kind: "result" | "ledger"): boolean {
  return (
    typeof value === "string" &&
    new RegExp(
      `^napier-research-benchmark-${kind}-[a-z][a-z0-9_]{2,80}-[a-f0-9]{16}\\.json$`,
      "u",
    ).test(value)
  );
}
