import {
  parseResearchSourceEvidenceV1 as parseResearchSourceEvidence,
  type ResearchSourceEvidenceV1,
  type SourceEvidence,
} from "./skill-load-research.js";
import {
  canonical,
  codeUnit,
  exact,
...
export const SKILL_LOAD_FAILURE_CODES = [
  "skill_not_enabled", "skill_not_found", "skill_ambiguous", "skill_disabled",
  "skill_invalid", "skill_untrusted", "skill_catalog_drift",
  "skill_limit_exceeded", "skill_load_cancelled",
] as const;
...
type Sha = string;
type RequestRecord =
  | { position: number; requestedNameSha256: Sha; state: "loadable"; canonicalName: string }
  | { position: number; requestedNameSha256: Sha; state: "unavailable"; failureContentSha256: Sha; canonicalName?: string };

export interface SkillLoadReceiptV1 {
  kind: "napier.skill-load-receipt"; schemaVersion: 1; operation: "skill.load";
  agentToolName: "skill_load"; state: "loaded"; name: string;
  requestedNameSha256: Sha; source: "project"; relativePath: string;
  sizeBytes: number; lineCount: number; rawContentSha256: Sha;
  invocationSha256: Sha; catalogSha256: Sha; snapshotManifestSha256: Sha;
  contentSha256: Sha;
}
type RequestFailure = {
  kind: "napier.skill-load-failure"; schemaVersion: 1; operation: "skill.load";
  agentToolName: "skill_load"; source: "project"; subject: "skill_request";
  state: "failed" | "unavailable"; failureCode: SkillLoadFailureCode;
  requestedNameSha256: Sha; canonicalName?: string; catalogSha256: Sha;
  snapshotManifestSha256?: Sha; diagnosticSha256: Sha; contentSha256: Sha;
};
type CatalogFailure = {
  kind: "napier.skill-load-failure"; schemaVersion: 1; operation: "skill.load";
  agentToolName: "skill_load"; source: "project"; subject: "project_catalog";
  state: "unavailable"; failureCode: "skill_limit_exceeded";
  observedDirectoryCount: 65; directoryIdentitySetSha256: Sha;
  catalogSha256: Sha; diagnosticSha256: Sha; contentSha256: Sha;
};
export type SkillLoadFailureV1 = RequestFailure | CatalogFailure;
export interface SkillCatalogBindingV1 {
  kind: "napier.skill-catalog-binding"; schemaVersion: 1; operation: "skill.load";
  agentToolName: "skill_load"; configuredSkillRequests: RequestRecord[];
...
export interface SkillLoadSelectionV1 {
  kind: "napier.skill-load-selection"; schemaVersion: 1; operation: "skill.load";
  agentToolName: "skill_load"; state: "selected"; name: string;
  requestedNameSha256: Sha; source: "project"; catalogSha256: Sha;
  availabilitySetSha256: Sha; snapshotManifestSha256: Sha;
  inputSha256: Sha; contentSha256: Sha;
}
type ManifestEntry = {
  canonicalName: string; requestedNameSha256: Sha; relativePath: string;
  virtualPath: string; directoryKind: "directory"; fileKind: "regular_file";
  symlinkFree: true; sizeBytes: number; lineCount: number; rawContentSha256: Sha;
  metadataSha256: Sha; invocationSha256: Sha;
};
export interface ProjectSkillSnapshotManifestV1 {
  kind: "napier.project-skill-snapshot-manifest"; schemaVersion: 1;
  source: "project"; trustOrigin: "active_user_selected_project";
  workspaceIdentitySha256: Sha; trustPolicySha256: Sha;
  configuredSkillRequests: RequestRecord[]; selectionSha256: Sha;
  directDirectoryCount: number; directoryIdentitySetSha256: Sha;
  catalogSha256: Sha; availabilitySetSha256: Sha; entryCount: number;
  aggregateRawBytes: number; entries: ManifestEntry[];
  unavailableFailureContentSha256s: Sha[]; snapshotContentSha256: Sha;
  snapshotManifestSha256: Sha;
}
export interface SkillApplicationProjectionV1 {
  kind: "napier.skill-application-projection"; schemaVersion: 1; runId: string;
  requestedNameSha256: Sha; skillName?: string; operation: "skill.load";
  agentToolName: "skill_load"; source: "project"; state: SkillLifecycleState;
  catalogSha256: Sha; availabilitySetSha256: Sha; snapshotManifestSha256: Sha;
  contextSeq: number; selectedSeq?: number; terminalSeq?: number;
  captureSeq?: number; citeSeq?: number; applicationSeq?: number; callId?: string;
  receiptContentSha256?: Sha; failureContentSha256?: Sha;
  applicationMode?: "citation_adjacent" | "verified_report";
  citationTokenSha256?: Sha; reportFileSha256?: Sha;
  reportCitationSetSha256?: Sha; projectionSha256: Sha;
}
...
function isProjectCatalogFailure(value: Record<string, unknown>): boolean {
  return exact(value,["kind","schemaVersion","operation","agentToolName","source","subject","state","failureCode","observedDirectoryCount","directoryIdentitySetSha256","catalogSha256","diagnosticSha256","contentSha256"]) && value.state === "unavailable" && value.failureCode === "skill_limit_exceeded" && value.observedDirectoryCount === 65 && hex(value.directoryIdentitySetSha256) && value.catalogSha256===sha256(canonical({directDirectoryCount:65,directoryIdentitySetSha256:value.directoryIdentitySetSha256})) && hashed(value,"contentSha256");
}
...
export function isSkillCatalogBindingV1(value: unknown): value is SkillCatalogBindingV1 {
  ...
  const requests=value.configuredSkillRequests as RequestRecord[], failures = value.unavailableSkills as SkillLoadFailureV1[]; if (failures.some((item) => item.subject !== "skill_request" || item.state !== "unavailable" || item.catalogSha256 !== value.catalogSha256) || !sortedUnique(failures.map((item) => item.contentSha256)) || !requestFailureRelations(requests,failures)) return false;
  const loadable=requests.filter((item)=>item.state==="loadable").map((item)=>item.canonicalName).sort(codeUnit); if(canonical(loadable)!==canonical(value.loadableSkillNames)) return false;
  const availability = { configuredSkillRequests: requests, loadableSkillNames: value.loadableSkillNames, unavailableFailureContentSha256s: failures.map((item) => item.contentSha256), catalogSha256: value.catalogSha256 };
...
function validManifestRelations(value:ProjectSkillSnapshotManifestV1):boolean {
  const requests=value.configuredSkillRequests, entries = value.entries as unknown as Record<string, unknown>[], names=entries.map((entry)=>String(entry.canonicalName)); if (!sortedUnique(names) || entries.some((entry)=>!manifestEntry(entry)) || entries.reduce((sum,entry)=>sum+Number(entry.sizeBytes),0)!==value.aggregateRawBytes || entries.length>value.directDirectoryCount) return false;
  const loadable=requests.filter((item)=>item.state==="loadable"); if(canonical(loadable.map((item)=>item.canonicalName).sort(codeUnit))!==canonical(names) || loadable.some((item)=>!entries.some((entry)=>entry.canonicalName===item.canonicalName&&entry.requestedNameSha256===item.requestedNameSha256))) return false;
  const unavailable=[...new Set(requests.filter((item)=>item.state==="unavailable").map((item)=>item.failureContentSha256))].sort(codeUnit); if(canonical(unavailable)!==canonical(value.unavailableFailureContentSha256s)) return false;
  if(value.catalogSha256!==sha256(canonical({directDirectoryCount:value.directDirectoryCount,directoryIdentitySetSha256:value.directoryIdentitySetSha256,entries})) || value.availabilitySetSha256!==sha256(canonical({configuredSkillRequests:requests,loadableSkillNames:names,unavailableFailureContentSha256s:unavailable,catalogSha256:value.catalogSha256}))) return false;
...
function requestFailureRelations(requests:RequestRecord[],failures:SkillLoadFailureV1[]):boolean { const refs=[...new Set(requests.filter((item)=>item.state==="unavailable").map((item)=>item.failureContentSha256))].sort(codeUnit); if(canonical(refs)!==canonical(failures.map((item)=>item.contentSha256))) return false; return requests.filter((item)=>item.state==="unavailable").every((request)=>{const failure=failures.find((item)=>item.contentSha256===request.failureContentSha256);return failure?.subject==="skill_request"&&failure.requestedNameSha256===request.requestedNameSha256&&failure.canonicalName===request.canonicalName;}); }
...
type ProjectionCommon = Omit<
  ProjectionInput,
  "state" | "selectedSeq" | "terminalSeq" | "captureSeq" | "citeSeq" |
  "applicationSeq" | "callId" | "receiptContentSha256" |
  "failureContentSha256" | "applicationMode" | "citationTokenSha256" |
  "reportFileSha256" | "reportCitationSetSha256"
>;
...
type LoadAttempt = {
  selected: ProjectionInput & { state: "selected"; selectedSeq: number; callId: string };
  terminal?: Record<string, unknown>;
  endSeq: number;
};

export function projectSkillApplicationV1(
  events: readonly unknown[],
  runId: string,
  target: { canonicalName: string } | { requestedNameSha256: string },
): SkillApplicationProjectionV1 | undefined {
...
function resolveProjectionTarget(
  runId: string,
  target: { canonicalName: string } | { requestedNameSha256: string },
): { canonicalName?: string; digest: string } | undefined {
  if (!/\S/u.test(runId) || runId.length > 160 || !object(target)) return;
  const targetKeys = Object.hasOwn(target, "canonicalName")
    ? ["canonicalName"]
    : ["requestedNameSha256"];
  if (!exact(target, targetKeys)) return;
  const targetName = "canonicalName" in target ? target.canonicalName : undefined;
  const targetDigest = "requestedNameSha256" in target
    ? target.requestedNameSha256
    : undefined;
  const canonicalName = name(targetName) ? targetName : undefined;
  const digest = canonicalName
    ? sha256(canonicalName)
    : hex(targetDigest)
      ? targetDigest
      : undefined;
...
function resolveProjectionContext(
  events: readonly unknown[],
  runId: string,
  target: { canonicalName?: string; digest: string },
  ...
  const request = binding.configuredSkillRequests.find(
    (item) =>
      item.requestedNameSha256 === target.digest &&
      (target.canonicalName === undefined ||
        item.canonicalName === target.canonicalName),
  );
...
function strictRunEvents(events:readonly unknown[],runId:string):Record<string,unknown>[]|undefined { const ordered:Record<string,unknown>[]=[]; let previous=0; for(const event of events){if(!object(event)||event.runId!==runId||!integer(event.seq,1,Number.MAX_SAFE_INTEGER)||Number(event.seq)<=previous)return;previous=Number(event.seq);ordered.push(event);}return ordered; }

function projectLoadLifecycle(
  ...
  for(const attempt of attempts){
    if(!attempt.terminal){latest=projection(attempt.selected);continue;}
...
function collectLoadAttempts(ordered:Record<string,unknown>[],binding:SkillCatalogBindingV1,common:ProjectionCommon):LoadAttempt[]|undefined {
  const starts:{event:Record<string,unknown>;selection:SkillLoadSelectionV1;payload:Record<string,unknown>}[]=[];
  for(const event of ordered){if(Number(event.seq)<=common.contextSeq||event.type!=="tool.started")continue;if(!object(event.payload)||event.payload.toolName!=="skill_load")continue;if(!isSkillLoadSelectionV1(event.payload.details))return;const selection=event.payload.details;if(selection.requestedNameSha256!==common.requestedNameSha256)continue;if(!selectionBindsContext(selection,event.payload,binding))return;starts.push({event,selection,payload:event.payload});}
  const callIds=starts.map((item)=>String(item.payload.callId));if(new Set(callIds).size!==callIds.length)return;
  const attempts:LoadAttempt[]=[];
  for(const [index,start] of starts.entries()){const endSeq=index+1<starts.length?Number(starts[index+1]!.event.seq):Number.POSITIVE_INFINITY;const terminals=ordered.filter((event)=>(event.type==="tool.completed"||event.type==="tool.failed")&&object(event.payload)&&event.payload.toolName==="skill_load"&&event.payload.callId===start.payload.callId);if(terminals.length>1||terminals.some((event)=>Number(event.seq)<=Number(start.event.seq)||Number(event.seq)>=endSeq))return;attempts.push({selected:{...common,state:"selected",selectedSeq:Number(start.event.seq),callId:String(start.payload.callId)},...(terminals[0]?{terminal:terminals[0]}:{}),endSeq});}
  return attempts;
}

function selectionBindsContext(
  selection: SkillLoadSelectionV1,
  payload: Record<string, unknown>,
  binding: SkillCatalogBindingV1,
): boolean {
  return (
    selection.catalogSha256 === binding.catalogSha256 &&
    selection.availabilitySetSha256 === binding.availabilitySetSha256 &&
    selection.snapshotManifestSha256 === binding.snapshotManifestSha256 &&
    typeof payload.callId === "string" &&
    /\S/u.test(payload.callId) &&
    payload.callId.length <= 160
  );
}

function projectLoadTerminal(
  ordered: Record<string, unknown>[],
  binding: SkillCatalogBindingV1,
  request: RequestRecord,
  attempt: LoadAttempt,
  ...
  if (terminal.type === "tool.failed") {
    if (
      !isSkillLoadFailureV1(details) ||
      details.subject !== "skill_request" ||
      details.state !== "failed" ||
      details.requestedNameSha256 !== request.requestedNameSha256 ||
      details.canonicalName !== request.canonicalName ||
  ...
  if (
    ...
    details.name !== request.canonicalName ||
    details.requestedNameSha256 !== request.requestedNameSha256 ||
    details.catalogSha256 !== binding.catalogSha256 ||
    details.snapshotManifestSha256 !== binding.snapshotManifestSha256
  ) {
  ...
  return request.canonicalName === "research-brief"
    ? projectResearchApplication(ordered, loaded, endSeq)
    : projection(loaded);
}
...
function projectResearchApplication(
  ...
  const capture = research.find(
    (item) =>
      item.evidence.action === "capture" ||
      item.evidence.action === "capture_fetch",
  );
  if (!capture) return projection(loaded);
  const source = capture.evidence as SourceEvidence;
  const cite = research.find(
    (item) =>
      Number(item.event.seq) > Number(capture.event.seq) &&
      item.evidence.action === "cite" &&
      (item.evidence as SourceEvidence).sourceId === source.sourceId &&
      (item.evidence as SourceEvidence).sourceContentSha256 ===
  ...
  const report = research.find(
    (item) =>
      Number(item.event.seq) > Number(cite.event.seq) &&
      item.evidence.action === "verify_report",
  );
  if (report) {
    if (!isExactApplicationChain(research, capture, cite, Number(report.event.seq))) {
      return projection(loaded);
    }
    const evidence = report.evidence as Extract<
      ResearchSourceEvidenceV1,
...
function collectResearchEvents(
  ordered: Record<string, unknown>[],
  terminalSeq: number,
  endSeq: number,
...
function isExactApplicationChain(
  research: ResearchEvent[],
  capture: ResearchEvent,
  cite: ResearchEvent,
  applicationSeq: number,
): boolean {
  const chain = research.filter(
    (item) =>
      Number(item.event.seq) < applicationSeq &&
      (item.evidence.action === "capture" ||
        item.evidence.action === "capture_fetch" ||
        item.evidence.action === "cite"),
  );
...
function citationAdjacent(text: string, tokenSha: string): boolean { const lines=text.split(/\r?\n/u); return lines.some((line,index)=>index>0&&/^\[citation:citation_[a-z0-9]{8,80}\]$/u.test(line.trim())&&lines[index-1]!.trim().length>0&&sha256(line.trim())===tokenSha); }
function projection<T extends Omit<SkillApplicationProjectionV1,"projectionSha256">>(value:T):SkillApplicationProjectionV1 { return {...value,projectionSha256:sha256(canonical(value))} as SkillApplicationProjectionV1; }
