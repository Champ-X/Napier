...
type Sha = string;
type ReportArtifactRegistration =
  | "registered"
  | "no_run_bound_plan"
...
type EvidenceCommon<Action extends string> = {
  kind: "napier.research-source-evidence"; schemaVersion: 1; action: Action;
  sourceCount: number; citationCount: number; sourceSetSha256: Sha;
  inputContentSha256: Sha; continuityCapsuleContentSha256?: Sha;
};
type SourceCommon = {
  sourceId: string; sourceContentSha256: Sha;
  sourceUrlSha256: Sha; sourceOriginSha256: Sha; sourceTitleSha256: Sha;
  sourceTextSha256: Sha; sourceLineCount: number; sourceTextChars: number;
  sourceTruncated: boolean;
};
type BrowserProvenance = {
  sourceKind: "browser"; browserSessionOperation: number;
  browserSessionIdSha256: Sha; browserActiveTabIdSha256: Sha;
  browserTabCount: number; browserTabSetSha256: Sha;
  browserExecutableSha256: Sha; browserVersionSha256: Sha;
  browserLimitsSha256: Sha; browserNetworkDestinationsSha256: Sha;
};
type WebBase = {
  sourceKind: "web_fetch"; webSourceContentSha256: Sha;
...
type WebUnavailable = WebBase & {
  webSourceFormat: "html"; webSourceRenderMode: "static";
  browserFallbackStatus: "unavailable";
  browserFallbackDiagnostic:
    | "browser_unavailable" | "browser_render_not_useful"
    | "fallback_limit_reached" | "login_required" | "challenge_detected";
};
type WebFallback = WebBase & {
  webSourceFormat: "html"; webSourceRenderMode: "browser_fallback";
  browserFallbackStatus: "used"; webFetchBrowserSessionOperation: number;
  webFetchBrowserSessionIdSha256: Sha; webFetchBrowserActiveTabIdSha256: Sha;
  webFetchBrowserTabCount: number; webFetchBrowserTabSetSha256: Sha;
  webFetchBrowserExecutableSha256: Sha; webFetchBrowserVersionSha256: Sha;
  webFetchBrowserLimitsSha256: Sha; webFetchBrowserNetworkDestinationsSha256: Sha;
};
type WebProvenance = WebStatic | WebUnavailable | WebFallback;
type Citation = {
  citationId: string; citationTokenSha256: Sha; citationStartLine: number;
  citationEndLine: number; citationQuoteSha256: Sha; citationClaimSha256: Sha;
};
type BrowserCaptureEvidence =
  EvidenceCommon<"capture"> & SourceCommon & BrowserProvenance;
type WebCaptureEvidence =
  EvidenceCommon<"capture_fetch"> & SourceCommon & WebProvenance;
type BrowserCitationEvidence =
  EvidenceCommon<"cite"> & SourceCommon & BrowserProvenance & Citation;
type WebCitationEvidence =
  EvidenceCommon<"cite"> & SourceCommon & WebProvenance & Citation;
export type SourceEvidence =
  | BrowserCaptureEvidence | WebCaptureEvidence
  | BrowserCitationEvidence | WebCitationEvidence;
export type ResearchSourceEvidenceV1 =
  | EvidenceCommon<"list">
  | BrowserCaptureEvidence | WebCaptureEvidence
  | BrowserCitationEvidence | WebCitationEvidence
  | (EvidenceCommon<"verify_report"> & { reportPathSha256: Sha;
      reportFileSha256: Sha; reportFileBytes: number; reportCitationCount: number;
...
type Provenance = { keys: string[]; normalized: BrowserProvenance | WebProvenance };
...
function browserProvenance(value: Record<string, unknown>): Provenance | undefined {
  if (!integer(value.browserSessionOperation,1,64) || !integer(value.browserTabCount,1,4) || typeof value.browserActiveTabId!=="string" || !/^tab_[1-9][0-9]{0,3}$/u.test(value.browserActiveTabId) || BROWSER.slice(1).filter((key)=>key!=="browserActiveTabId"&&key!=="browserTabCount").some((key)=>!hex(value[key]))) return;
  return { keys:BROWSER, normalized:{sourceKind:"browser",browserSessionOperation:Number(value.browserSessionOperation),browserSessionIdSha256:String(value.browserSessionIdSha256),browserActiveTabIdSha256:sha256(value.browserActiveTabId),browserTabCount:Number(value.browserTabCount),browserTabSetSha256:String(value.browserTabSetSha256),browserExecutableSha256:String(value.browserExecutableSha256),browserVersionSha256:String(value.browserVersionSha256),browserLimitsSha256:String(value.browserLimitsSha256),browserNetworkDestinationsSha256:String(value.browserNetworkDestinationsSha256)} };
}
...
function webProvenance(value: Record<string, unknown>): Provenance | undefined {
  const normalized = {sourceKind:"web_fetch" as const,webSourceContentSha256:value.webSourceContentSha256 as string,webSourceBodySha256:value.webSourceBodySha256 as string,webSourceFormat:value.webSourceFormat as WebBase["webSourceFormat"],webSourceLineCount:value.webSourceLineCount as number};
  if (FALLBACK.every((key)=>value[key]===undefined)) return {keys:[...WEB],normalized:{...normalized,webSourceRenderMode:"static",browserFallbackStatus:"not_needed"}};
  const mode=value.webSourceRenderMode, status=value.browserFallbackStatus;
  const keys=[...WEB,"webSourceRenderMode","browserFallbackStatus"];
  if (mode==="static" && status==="not_needed" && value.browserFallbackDiagnostic===undefined && FALLBACK.slice(3).every((key)=>value[key]===undefined)) return {keys,normalized:{...normalized,webSourceRenderMode:"static",browserFallbackStatus:"not_needed"}};
  if (mode==="static" && status==="unavailable" && value.webSourceFormat==="html" && DIAGNOSTICS.has(String(value.browserFallbackDiagnostic)) && FALLBACK.slice(3).every((key)=>value[key]===undefined)) return {keys:[...keys,"browserFallbackDiagnostic"],normalized:{...normalized,webSourceFormat:"html",webSourceRenderMode:"static",browserFallbackStatus:"unavailable",browserFallbackDiagnostic:value.browserFallbackDiagnostic as WebUnavailable["browserFallbackDiagnostic"]}};
  return browserFallbackProvenance(value, keys, normalized);
}

function browserFallbackProvenance(
  value: Record<string, unknown>,
  keys: string[],
  normalized: WebBase,
): Provenance | undefined {
  if (value.webSourceRenderMode!=="browser_fallback" || value.browserFallbackStatus!=="used" || value.webSourceFormat!=="html" || value.browserFallbackDiagnostic!==undefined || !integer(value.webFetchBrowserSessionOperation,1,64) || !integer(value.webFetchBrowserTabCount,1,4) || typeof value.webFetchBrowserActiveTabId!=="string" || !/^tab_[1-9][0-9]{0,3}$/u.test(value.webFetchBrowserActiveTabId) || FALLBACK.slice(4).filter((key)=>key!=="webFetchBrowserActiveTabId"&&key!=="webFetchBrowserTabCount").some((key)=>!hex(value[key]))) return;
  return {keys:[...keys,...FALLBACK.slice(3)],normalized:{...normalized,webSourceFormat:"html",webSourceRenderMode:"browser_fallback",browserFallbackStatus:"used",webFetchBrowserSessionOperation:Number(value.webFetchBrowserSessionOperation),webFetchBrowserSessionIdSha256:String(value.webFetchBrowserSessionIdSha256),webFetchBrowserActiveTabIdSha256:sha256(value.webFetchBrowserActiveTabId),webFetchBrowserTabCount:Number(value.webFetchBrowserTabCount),webFetchBrowserTabSetSha256:String(value.webFetchBrowserTabSetSha256),webFetchBrowserExecutableSha256:String(value.webFetchBrowserExecutableSha256),webFetchBrowserVersionSha256:String(value.webFetchBrowserVersionSha256),webFetchBrowserLimitsSha256:String(value.webFetchBrowserLimitsSha256),webFetchBrowserNetworkDestinationsSha256:String(value.webFetchBrowserNetworkDestinationsSha256)}};
}
...
export function parseResearchSourceEvidenceV1(value: unknown): ResearchSourceEvidenceV1 | undefined {
  ...
  const base={kind:"napier.research-source-evidence" as const,schemaVersion:1 as const,action:input.action as EvidenceAction,sourceCount:Number(input.sourceCount),citationCount:Number(input.citationCount),sourceSetSha256:String(input.sourceSetSha256),inputContentSha256:sha256(canonical(input)),...(input.stateCapsule?{continuityCapsuleContentSha256:String((input.stateCapsule as Record<string,unknown>).capsuleSha256)}:{})};
...
function parseReport(
  ...
  base: EvidenceCommon<EvidenceAction>,
  optional: string[],
): ResearchSourceEvidenceV1 | undefined {
  const opt=[...optional,"reportArtifactRegistration"];
  if (!validReport(input, opt)) return;
  return {...base,action:"verify_report",reportPathSha256:input.reportPathSha256,reportFileSha256:input.reportFileSha256,reportFileBytes:input.reportFileBytes,reportCitationCount:input.reportCitationCount,reportCitationSetSha256:input.reportCitationSetSha256,...(input.reportArtifactRegistration?{reportArtifactRegistration:input.reportArtifactRegistration}:{})} as ResearchSourceEvidenceV1;
}
...
function parseSource(
  ...
  return {...base,action:input.action,sourceId:String(input.sourceId),sourceContentSha256:String(input.sourceContentSha256),sourceUrlSha256:String(input.sourceUrlSha256),sourceOriginSha256:String(input.sourceOriginSha256),sourceTitleSha256:String(input.sourceTitleSha256),sourceTextSha256:String(input.sourceTextSha256),sourceLineCount:Number(input.sourceLineCount),sourceTextChars:Number(input.sourceTextChars),sourceTruncated:Boolean(input.sourceTruncated),...prov.normalized,...(cite?Object.fromEntries(CITE.map((key)=>[key,input[key]])): {})} as ResearchSourceEvidenceV1;
}
