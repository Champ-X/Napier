...
import { validateModelInvocationExperimentComparison } from "./model-invocation-experiment-comparison-validation.js";
...
export function validateModelInvocationExperimentPreview(
  ...
  if (
    !validModelInvocationPreviewIdentity(value) ||
    !validModelInvocationPreviewEvidence(value)
  ) {
...
function validModelInvocationPreviewIdentity(
...
function validModelInvocationPreviewEvidence(
  value: Record<string, unknown>,
): boolean {
  return (
...
export function validateModelInvocationExperimentResult(
  ...
  if (
    !validModelInvocationResultIdentity(value, preview, assistantText) ||
    !validModelInvocationResultBinding(
      value,
      preview,
      comparison,
      candidateToolCallNames,
    ...
    throw new Error(
      "Model invocation experiment result binding is invalid (result is invalid)",
    );
  }
  return {
    kind: value["kind"],
...
function validModelInvocationResultIdentity(
  value: Record<string, unknown>,
...
function validModelInvocationResultBinding(
  value: Record<string, unknown>,
  preview: ModelInvocationExperimentPreview,
  comparison: ReturnType<typeof validateModelInvocationExperimentComparison>,
  ...
  return (
    ...
    canonicalJson(comparison.source.model) ===
      canonicalJson(preview.sourceModel) &&
    comparison.source.stopReason === preview.sourceStopReason &&
    comparison.source.textSha256 === preview.sourceTextSha256 &&
    ...
    canonicalJson(comparison.target.model) ===
