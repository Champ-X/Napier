...
/api/receipt-trust/anchors` and `POST /api/receipt-trust/anchors` promote the
trust-anchor list, create request, and `201` created-anchor response schemas so
external management clients can enroll verifier trust anchors against the same
contract used by the server and Web client. The compatibility fixture generated
by `scripts/check-management-openapi-compatibility.mjs` projects each published
