...
check-only guard against route drift. The compatibility fixture generated by
...
request-body presence, and response status set. Verification allows additive
operations while rejecting removed or changed published operations, giving
external management clients a baseline that is stricter than route discovery
but still additive-friendly.
...
- endpoint-level JSON Schema promotion for external management-plane clients.
