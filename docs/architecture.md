...
check-only guard against route drift. `GET /api/health` is the first promoted
endpoint schema: its `200` response references
`#/components/schemas/HealthResponse`, matching the shared contracts package
shape used by the server and Web client. The compatibility fixture generated by
...
- additional endpoint-level JSON Schema promotion for external management-plane
  clients.
