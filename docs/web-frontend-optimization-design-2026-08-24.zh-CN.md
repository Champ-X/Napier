...
- 运行现有 Web 设计契约检查，当前结果为通过，但仍记录 841 处颜色字面量债务。
...
| reading range | 760px 至 840px | min 640px，target 800px，max 880px | 区分布局硬下限与理想阅读宽度 |
...
v1.1 保留现有消费方依赖的 token 名称，同时新增范围和组件 token。后续删除旧 token 时应保留一个发布周期的 deprecated 窗口。生成脚本已经同步输出新 token，颜色债务仍按 841 处基线渐进收敛，避免一次替换造成高风险大提交。
...
- 当前 841 处颜色字面量债务在首个重构版本降至 200 以下，后续目标为零。
