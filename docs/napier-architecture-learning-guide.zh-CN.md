...
维护日期：2026-09-09，当前说明对照源码基线 `02ff34a5`。本文用于学习架构、机制与取舍；当前检查结果统一见[缺口矩阵](next-stage-gap-matrix.md)，操作与实现契约见[架构说明](architecture.md)。

阅读建议：先理解第 1～4 部分，再沿源码阅读其他机制，最后用第 17～19 部分进行面试演练。早期题库和旧验收记录可从[Git 历史索引](archive/README.md)查阅。
...
进展向量 v3 分别记录语义进展、产物就绪与 activity。新的观察和不同产物状态可以支持有限的继续执行窗口，但不会自动成为验收通过。收敛控制区分工具内部重试与模型策略回合；计划更新声明为 coordinate/control。窗口起点与上限由持久证据约束，恢复不会重置。相同参数和相同结果反复出现时，Tool Loop Guard 仍会改向或阻止重复执行。具体阈值与来源见[进展控制](architecture.md#run-budget-and-progress-flow)。
...
同一 Run 的长执行链还有独立的滚动压缩：工具结果裁剪后、最终 token governor 前，按实际服务模型的输入预算检查压力，把较旧的完整执行单元合并到增量摘要。用户消息和最近完整工具批次保留，检查点绑定原始来源、父节点和摘要响应；压缩不重放工具、不重置预算，也不增加语义进展。两条路径的触发条件与事件见[上下文压缩](architecture.md#context-compaction-flow)。

工具结果也会针对空内容、过大的内容、重复错误或被新读取取代的旧结果进行裁剪。最终调用仍经过 token pressure 检查。上下文收缩改变模型输入投影，原始 Pi transcript、账本与工具胶囊继续保留。
...
[当前缺口矩阵](next-stage-gap-matrix.md)统一记录检查时间、源码基线、失败与关闭条件；[本地开发说明](local-development.md#development-checks)区分开发构建和发布验收。这里不维护另一套测试数量、性能数据或架构豁免清单。

面试讨论时，应区分确定性测试、故障注入、真实模型任务和发布验收。预算、公开 API、兼容性与视觉规则分别由对应文件和检查维护，某个检查通过不能替代其他证据。

适合沿源码讨论的取舍包括：

1. 如何通过稳定领域接口减少 AgentRuntime、Store 和 Workspace ViewModel 的变更影响面。
2. 如何测量长 Thread 的增量查询、snapshot 传输、浏览器内存与 checkpoint 成本。
3. 同步 SQLite 写入、状态事务与锁竞争适合什么运行规模，以及何时值得改变存储架构。
4. 各类副作用的幂等、补偿与未知状态边界，为什么不能共用一个无条件 retry 策略。
5. 如何用同模型、多次执行和有代表性的任务区分 Harness 改进、模型波动与环境变化。
