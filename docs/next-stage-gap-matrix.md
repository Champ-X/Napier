...
| P3 browser/research/data/media    | Partial        | Run-owned Chrome supports controlled interaction and Artifact movement. Research Sources provide claim-bound citations and verified Markdown; exact fetched final URLs can auto-settle declared URL Artifacts from normalized Source evidence. Data analysis includes flat-file inspection, typed DataFrame transformations, read-only SQLite, and deterministic SVG delivery with verified Artifacts. Durable raw Source copies, unified upload/Source/file/download models, source-quality and contradiction automation, package-backed Python/Notebook, interactive visualization, browser UX, and media production remain. |
