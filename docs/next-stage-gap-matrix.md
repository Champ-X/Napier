...
| P3 browser/research/data/media    | Partial        | Run-owned Chrome supports interaction and Artifact movement. Research Sources provide citations and verified Markdown; exact URLs auto-settle URL Artifacts, and Safe Automation saves exact bounded raw HTML/PDF/text/JSON/Markdown bytes into declared verified files. Data includes inspection, DataFrame, read-only SQLite, and SVG delivery. Unified upload/Source/file/download identity, raw Source versions/refresh, broader documents/media, source-quality automation, package-backed Python/Notebook, interactive visualization, and browser UX remain. |
