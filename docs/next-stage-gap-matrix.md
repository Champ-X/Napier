...
- Graceful shutdown terminates every active session before Store close.
  ...
  Linux uses Bubblewrap's native parent-death contract. Both terminate the
  complete target group; startup still records interruption because cleanup
  does not prove the task outcome. Cross-restart reattachment requires a
  durable managed identity and remains outside this slice.
...
- Guardian tests use a real detached process tree, kill its Runtime parent with
  `SIGKILL`, and prove both guardian and target disappear. They also cover
  normal IO, target-spawn denial, terminal-spec privacy, and resource-evidence
  binding. A fresh built Runtime repeated the abrupt-parent cleanup path.
