...
- `apps/server/src/app.ts` and `packages/runtime/src/store.ts` remain the
  largest production modules. Memory HTTP, bounded request-body reads, and
  response evidence are now extracted; new execution code must stay outside
  both oversized composition modules.
...
| --------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0 architecture and baseline      | In progress    | Checked architecture budgets now freeze production/test module growth, per-file maximum function complexity, root and extracted-domain public exports, workspace dependency direction, and one remaining two-module Receipt Trust component, down from 10 components and 54 cyclic Runtime modules to 2 cyclic modules. Leaf domain models, the 133-declaration Contracts execution extraction, Event/client/mutation/Store SPIs, and the Run replay domain removed the other 52 Runtime modules from cycles while preserving public APIs and the single authoritative Store. The local product-path budget covers built CLI startup/first token/completion, shared Runtime bootstrap, production read-tool latency, 1,000-event append/projection, observed RSS, and closed SQLite bytes/event. Complete the multi-module Receipt Trust validator split; extend performance budgets to external Providers, HTTP/browser paths, 10,000-event Threads, and enforced resource quotas.                                                                                                                                                                                                                                                                                                                                              |
...
| P9 outcome benchmark              | In progress    | Three fixed Coding cases cover single-file repair, multi-file LSP migration, and debugger-qualified repair with repeated trials, Sandbox assertions, tool distributions, and Ledger evidence. Workflow, Data, and SQLite prompt-injection Security cases now measure real model-backed typed Map fan-out plus deterministic Reduce, exact outputs, Run isolation, Replay validity, credential privacy, SQLite action protocol, database immutability, hash-bound injection-row exposure, output leakage, offline Result/Ledger verification, and repeated latency/cost/token distributions. DeepSeek passed all three retained two-trial Series 2/2; these small samples are not a cross-model superiority claim. Research, Long-horizon, broader Security, and UX cases, broader Coding/Workflow/Data tasks, reference-project execution, and larger cross-model distributions remain.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- the formal release audit semantically verifies the Series and both
  ...
  22-artifact release receipt. Missing, substituted, or tampered trials block
...
- release verification independently loads all three Benchmark Series plus all
  twelve referenced Result/Ledger files and binds the fifteen physical
  benchmark files into the 22-artifact release receipt.
